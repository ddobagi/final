...
export default function ReplyDetail() {
    ...
    useEffect(() => {
        ...
        const match = referrer.match(/\/dashboard\/([^/]+)/);
        if (match) {
        const firstSlug = match[1]; // 첫 번째 slug 값
        console.log("추출한 firstSlug:", firstSlug);
        }
        setFirstSlug(firstSlug)
    ...
    const fetchVideoData = async (slug, mode) => {
        ...
        try {
            ...
            let docRef = doc(db, "gallery", slug);
...
export default function VideoDetail() {
  ...
  const fetchVideoData = async (slug, mode) => {
    ...
    try {
        ...
        // mode 값에 따라 상이한 db 경로에서 문서를 불러옴 
        const userId = auth.currentUser?.uid;
        let docRef = mode ? doc(db, "gallery", firstSlug, "comment", secondSlug) : doc(db, "gallery", firstSlug, "comment", secondSlug);
        const docSnap = await getDoc(docRef);
